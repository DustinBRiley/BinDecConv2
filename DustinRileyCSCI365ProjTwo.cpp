...
string signed_extension(string s){  // extends binary out to 16 bits keeping its sign
    // you implement this one first
    if (s[0] == '0') {              // if first bit is 0 binary is positive extend out to 16 bits
        while(s.length() < 16) {
            s = "0" + s;
        }
        return s;
        ...
        while(s.length() < 16) {    // binary is negative extend out with 1s to 16 bits
            s = "1" + s;
        }
        return s;
    }
...
int binary_to_decimal_signed(string s){
    ...
    bool neg = false;               // negative check boolean
    if (s[0] == '1') {              // if binary starts with 1 it is negative and needs to be converted to its twos complement (positive binary)
        neg = true;                 // binary is negative set negative check to true
        s = twos_complement(s);     // convert to binary to its twos complement
    }
    int dec = binary_to_decimal(s); // convert positive binary to decimal
    if (neg) {                      // if negative check is true convert decimal to negative
        return dec - 2 * dec;
    }
    return dec;
}
...
string decimal_to_binary_signed(int n){
    ...
    bool neg = false;   // negative check boolean
    if (n < 0) {        // if decimal is negative set check to true and convert decimal to positive
        neg = true;     // set negative check to true
        n = n - 2 * n;  // convert decimal to its positive
    }
    string str = decimal_to_binary(n);  // convert decimal to binary
    while(str.length() < 16) {
        str = "0" + str;
    }
    if (neg) {                          // if negative check is true convert binary to its twos complement
        return twos_complement(str);
    }
    str = signed_extension(str);        // signed extend binary
    return str;
}
...
string add_binaries_signed(string b1, string b2){
    ...
    if (b1[0] == '1' && b2[0] == '1') {     // if both negative switch both to twos complement
        b1 = twos_complement(b1);
        b2 = twos_complement(b2);
        string str = add_binaries(b1, b2);  // add them together
        str = twos_complement(str);         // then change back to negative
        return str;
    }
    return add_binaries(b1, b2);            // add them together
}

string twos_complement(string s){
    // you implement this one second
    string str = signed_extension(s);   // extend to 16 bits
    if (str == "0000000000000000") {    // 0 case
        return str;
    }
    for (int i = 0;i < 16;i++) {        // reverse all bits i.e. ("111" -> "000")
        if(str[i] == '0') {
            str[i] = '1';
        }
        else {
    ...
    string str2 = "1";
    str = add_binaries(str, str2);      // add 1 to binary
    return str;
}
